...
def vintage_payload(certified=True, lineage=None):
    results = []
    for index, (sid, ticker, score) in enumerate((("WT-0001", "AAA", 90.0), ("WT-0002", "BBB", 80.0)), 1):
        results.append({
            ...
            "category_scores": {"QUALITY": score},
            "flags": [],
            "overall_rank": index,
            "rank_status": "WATCH",
            "available_at": "2026-01-31T00:00:00Z",
        })
    vintage = {
        "information_cutoff": "2026-02-01T00:00:00Z",
        "generated_at": "2026-02-01T00:00:00Z",
        "certification": {"status": "CERTIFIED" if certified else "BLOCKED"},
        "lineage": m13_lineage() if lineage is None else lineage,
        "results": results,
    }
...
def files(tmp_path):
    ...
    write_json(vintage, vintage_payload())
    cfg = tmp_path / "portfolio.json"
    write_json(cfg, {
        "portfolio": {"holdings_count": 1, "reserves_count": 1},
        "concentration": {
            "max_emerging_positions": 1,
            "max_per_universe_group": 2,
            "max_per_primary_theme": 2,
            "max_per_business_stage": 2,
            "max_per_economic_exposure": 2,
        },
        "rebalance": {
            "holding_buffer_rank": 1,
            "maximum_score_gap_to_cutoff": 0.0,
...
def test_future_cutoff_fails_closed():
    ...
    payload["vintages"][0]["information_cutoff"] = "2026-02-02T00:00:00Z"
...
def test_duplicate_rank_fails_closed():
    payload = vintage_payload()
    payload["vintages"][0]["results"][1]["overall_rank"] = 1
    with pytest.raises(ShadowPortfolioError, match="SHADOW_PORTFOLIO_DUPLICATE_RANK"):
        certify_vintage(payload, as_of_date="2026-02-01")


def test_m13_named_hash_lineage_is_accepted():
    result = certify_vintage(vintage_payload(), as_of_date="2026-02-01")
    assert result["status"] == "CERTIFIED"


def test_incomplete_m13_lineage_fails_closed():
    payload = vintage_payload(lineage={"pipeline_version": "1.0.0"})
    with pytest.raises(ShadowPortfolioError, match="SHADOW_PORTFOLIO_LINEAGE_INCOMPLETE"):
        certify_vintage(payload, as_of_date="2026-02-01")


def test_legacy_lineage_list_remains_compatible():
    payload = vintage_payload(lineage=[{"name": "source", "sha256": "a" * 64}])
    assert certify_vintage(payload, as_of_date="2026-02-01")["status"] == "CERTIFIED"
...
def test_end_to_end_shadow_portfolio_uses_real_portfolio_cli(tmp_path):
    ...
    assert result["portfolio"]["source_scoring_configuration_sha256"] == "1" * 64
